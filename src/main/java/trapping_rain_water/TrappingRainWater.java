 ...
 *
 * Follow-up:
 * If the elevation is 0, it is leaking the water (no traping). 
 */
...
public class TrappingRainWater {
	public int trap(int[] A) {
	   ...
	   public int trapWithLeaking(int[] A) {
	        /*
	         * For each bar, find the maximum height at the left and right side. 
	         * The water trapped by this bar is min(max_left, max_right) - height.
	         * Time Complexity O(n)
	        ...
	        int[] maxLeft = new int[A.length];
	        for (int i = 0; i < A.length; i++) {
	            currMaxLeft = A[i] == 0 ? 0 : Math.max(currMaxLeft, A[i]);
	            maxLeft[i] = currMaxLeft;
	        }
	        for (int i = A.length - 1; i >= 0; i--) {
	            currMaxRight = A[i] == 0 ? 0 : Math.max(currMaxRight, A[i]);
	            int height = Math.min(maxLeft[i], currMaxRight);
	            if (height > A[i])
	                waterTrapped += height - A[i];
	        }
